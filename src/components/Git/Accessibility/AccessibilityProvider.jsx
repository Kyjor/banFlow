...
export function AccessibilityProvider({ children }) {
  ...
  const registerShortcut = useCallback((keys, callback, description) => {
    ...
    setShortcuts(
      (prev) => new Map(prev.set(keyString, { callback, description, keys })),
    );
  }, []);
  ...
  useEffect(() => {
    if (announcements.length > 0) {
      ...
      setTimeout(() => {
        setAnnouncements((prev) =>
          prev.filter((a) => a.timestamp !== latestAnnouncement.timestamp),
        );
      }, 1000);
    }
  }, [announcements]);
  ...
  const focusElement = useCallback(
    (selector) => {
      ...
      if (element) {
        element.focus();
        announce(
          `Focused: ${element.getAttribute('aria-label') || element.textContent || 'element'}`,
        );
      }
    },
    [announce],
